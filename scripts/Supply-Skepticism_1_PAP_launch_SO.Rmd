...
col.labels <- c("Units in Afford. Neighborhods","Units in Low Income Neighborhoods")
...
```{r results GD figs, fig.cap="All Respondents."}
...
```{r results GD econ, results='asis'}
...
all_terms <- c(terms.0, terms.1, terms.2, terms.3)

for (i in seq_along(yvar)){
terms <- paste0(yvar[i], " ", all_terms) 

terms %>% 
    map(~ lm(.x, data=D)) %>%
  modelsummary(title=paste0("All Respondents: DV = ",yvar[i]))  -> x

print(x)
  
  #   map_dfr(tidy, .id="model") %>%
  # ggplot(aes(x=estimate, y=factor(term),group=factor(model), color=factor(model))) + 
  # geom_point(aes(group=model,color=model), position=position_dodge(width=.8)) +
  # geom_errorbarh(aes(group=model,color=model,xmin=estimate-std.error,xmax=estimate+std.error), position=position_dodge(width=.8)) + 
  # geom_text_repel(aes(label=paste0(round(estimate,2), " (",round(std.error,2),")"))) +
  # theme_bw() +theme(legend.position = "none") +ggtitle(yvar[i]) 
...
```{r GD homeowners, fig.cap="Homeowners.", results="asis"}
all_terms <- c(terms.0, terms.1, terms.2, terms.3)
...
terms %>% 
    map(~ lm(.x, data=D %>% filter(ownhome==1))) %>%
  modelsummary(title=paste0("Homeowners: DV = ",yvar[i]))  -> x
...
terms %>% 
    map(~ lm(.x, data=D %>% filter(ownhome==0))) %>%
  modelsummary(title=paste0("Renters: DV = ",yvar[i]))  -> x
...
```{r notlower, fig.cap="Those Who Want Prices Higher/Same.", results='asis'}
...
terms %>% 
    map(~ lm(.x, data=D %>%  filter(want.price2 == "Not_lower"))) %>%
  modelsummary(title=paste0("Want Prices Higher/Same: DV = ",yvar[i]))  -> x
...
```{r pricesLower, fig.cap="Those Who Want Prices Lower.", results='asis'}
...
terms %>% 
    map(~ lm(.x, data=D %>%  filter(want.price2 == "Lower"))) %>%
  modelsummary(title=paste0("Want Prices Lower: DV = ",yvar[i]))  -> x
...
```{r results GD support models, results='asis'}
...
all_terms <- c(terms.1, terms.2, terms.3, terms.4)
...
terms <- paste0(yvar, " ", all_terms) 

terms %>% 
    map(~ lm(.x, data=D)) %>%
  modelsummary(title=paste0("All Respondents: DV = ",yvar))  -> x
...
```{r homeowners, fig.cap="Homeowners.", results='asis'}
...
terms %>% 
    map(~ lm(.x, data=D%>%
 filter(ownhome == 1) )) %>%
  modelsummary(title=paste0("Homeowners: DV = ",yvar))  -> x
...
```{r renters, fig.cap="Renters.", results='asis'}
...
terms %>% 
    map(~ lm(.x, data=D%>%
 filter(ownhome == 0) )) %>%
  modelsummary(title=paste0("Renters: DV = ",yvar))  -> x
...
terms %>% 
    map(~ lm(.x, data=D%>%
 filter(want.price2 == "Not_lower") )) %>%
  modelsummary(title=paste0("Want Prices Higher: DV = ",yvar))  -> x
...
```{r prices lower, results='asis'}

terms %>% 
    map(~ lm(.x, data=D%>%
 filter(want.price2 == "Lower") )) %>%
  modelsummary(title=paste0("Want Prices Lower: DV = ",yvar))  -> x
